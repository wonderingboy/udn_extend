...
template <typename Dtype>
class MaxMaskLayer : public Layer<Dtype> {
 public:
  /**
   * @param param provides ArgMaxParameter argmax_param,
   *     with ArgMaxLayer options:
   ...
   *     if set, output a vector of pairs (max_ind, max_val) for each image.
   */
  explicit MaxMaskLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "MaxMask"; }
...
template <typename Dtype>
class SliceBatchLayer : public Layer<Dtype> {
 public:
  explicit SliceBatchLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "SliceBatch"; }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int MinTopBlobs() const { return 2; }
...
class CorrelationLayer : public Layer<Dtype> {
 public:
  ...
  virtual inline const char* type() const { return "Correalation"; }
  /**
