...
template <typename Dtype>
class EuclideanPartLossLayer : public LossLayer<Dtype> {
 public:
  explicit EuclideanPartLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "EuclideanPartLoss"; }
  /**
   * Unlike most loss layers, in the EuclideanLossLayer we can backpropagate
   * to both inputs -- override to return true and always allow force_backward.
   */
 ...
 protected:
  ...
  Blob<Dtype> diff_;
  Blob<Dtype> mask_diff_;
};

template <typename Dtype>
class LogisticLossLayer : public LossLayer<Dtype> {
 public:
  explicit LogisticLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "LogisticLoss"; }
