...
#include "caffe/util/im2col_sk.hpp"
#include "caffe/util/math_functions.hpp"
#include "caffe/sk_layers.hpp"
...
void ConvolutionSKLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_EQ(stride_h_, 1) << "stride_h_ must be 1. The layer is designed to remove downsampling.";
  CHECK_EQ(stride_w_, 1) << "stride_w_ must be 1. The layer is designed to remove downsampling.";
  ...
  CHECK_GE(kstride_h_, 1) << "kstride_h_ must be no less than 1.";
  CHECK_GE(kstride_w_, 1) << "kstride_w_ must be no less than 1.";
...
void ConvolutionSKLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  height_out_ = (height_ + 2 * pad_h_ - ext_kernel_h_) / stride_h_ + 1;
  width_out_ = (width_ + 2 * pad_w_ - ext_kernel_w_) / stride_w_ + 1;
