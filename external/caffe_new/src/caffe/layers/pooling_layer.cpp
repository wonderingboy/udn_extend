...
void PoolingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if ( (this->layer_param_.pooling_param().pool() == PoolingParameter_PoolMethod_DEF) )//set flag in layer_setup
  {
...
*/      parts_num_ = 9;     // part
     ...
     unsigned int hpos_enum[3] = {2,3,4};
     for (int p = 0; p < parts_num_/3; ++p)
      {
          // 0 1
          // 2 3
          hpos_[p*3] = floor((p*height_)/3);  // save the part start location hpos_enum[p];//
          hpos_[p*3+1] = floor((p*height_)/3);
          hpos_[p*3+2] = floor((p*height_)/3);
          vpos_[p*3] = 0 ;
          vpos_[p*3+1] = floor((1*width_)/3);
          vpos_[p*3+2] = floor((2*width_)/3);
      }
    /*hpos_[0] = 2;
    hpos_[1] = 2;
    vpos_[0] = 2;
